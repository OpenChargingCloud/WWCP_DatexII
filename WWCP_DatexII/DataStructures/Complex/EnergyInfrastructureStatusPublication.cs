/*
 * Copyright (c) 2014-2025 GraphDefined GmbH <achim.friedland@graphdefined.com>
 * This file is part of WWCP DatexII <https://github.com/OpenChargingCloud/WWCP_DatexII>
 *
 * Licensed under the Affero GPL license, Version 3.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.gnu.org/licenses/agpl.html
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#region Usings

using System.Xml.Serialization;

#endregion

namespace cloud.charging.open.protocols.DatexII
{

    [XmlRoot("payload", Namespace = "http://datex2.eu/schema/3/d2Payload")]
    public class EnergyInfrastructureStatusPublication
    {

        [XmlElement(ElementName = "publicationTime",                Namespace = "http://datex2.eu/schema/3/common")]
        public DateTime?                        PublicationTime                   { get; set; }


        [XmlElement(ElementName = "publicationCreator",             Namespace = "http://datex2.eu/schema/3/common")]
        public PublicationCreator?              PublicationCreator                { get; set; }


        [XmlElement(ElementName = "tableReference",                 Namespace = "http://datex2.eu/schema/3/common")]
        public TableReference?                  TableReference                    { get; set; }


        [XmlElement(ElementName = "headerInformation",              Namespace = "http://datex2.eu/schema/3/d2Payload")]
        public HeaderInformation?               HeaderInformation                 { get; set; }


        [XmlElement(ElementName = "energyInfrastructureSiteStatus", Namespace = "http://datex2.eu/schema/3/energyInfrastructure")]
        public EnergyInfrastructureSiteStatus?  EnergyInfrastructureSiteStatus    { get; set; }

    }

}
